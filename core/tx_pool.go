...
var DefaultTxPoolConfig = TxPoolConfig{
	...
	AccountSlots: 16,
	GlobalSlots:  20000, //using to set the threshold of txpool
	AccountQueue: 64,
	GlobalQueue:  1024,
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, eventMux *event.TypeMux, currentStateFn stateFn, gasLimitFn func() *big.Int) *TxPool {
	...
	log.Info("in NewTxPool, call resetState")
	pool.resetState()

	// Start the various events loops and return
	pool.wg.Add(2)
	log.Info("in NewTxPool, call pool.eventLoog()")
	go pool.eventLoop()
	...
	for {
		...
		case ev, ok := <-pool.events.Chan():
			...
			case ChainHeadEvent:
				...
				log.Info("in pool.evenLoop, call pool.resetState")
				pool.resetState()
				pool.mu.Unlock()
