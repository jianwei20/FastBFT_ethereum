...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	bftValidatorCommitCh := make(chan *types.BFTValidatorBlock)
	if eth.protocolManager, err = NewProtocolManager(eth.chainConfig, config.SyncMode, config.NetworkId, maxPeers, eth.eventMux, eth.txPool, eth.engine, eth.blockchain, chainDb, bftValidatorCommitCh); err != nil {
		return nil, err
	}
	...
	if config.BFT {
		if bft, ok := eth.engine.(*bft.BFT); ok {
			...
			if err = bft.SetupProtocolManager(chainConfig, eth.protocolManager.networkId, eth.eventMux, eth.txPool, eth.blockchain, chainDb, bftDb, vmConfig, config.Validators, config.PrivateKeyHex, config.Etherbase, config.AllowEmpty, config.ByzantineMode, bftValidatorCommitCh); err != nil {
				return nil, err
			}
			bft.Start()
		}
