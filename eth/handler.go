...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, maxPeers int, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database, bftValidatorCommitCh chan *types.BFTValidatorBlock) (*ProtocolManager, error) {
...
func(pm *ProtocolManager) BFTvalidatorCommitBlock() {
	log.Info("---In BFTvalidatorCommitBlock---")
	var result *types.BFTValidatorBlock
	var block *types.Block
	var peerId string
	for {
		select {
		case result = <-pm.bftValidatorCommitCh:
			log.Info("**in eth.handler**, get result from ch")
			peerId, block = result.PeerId, result.Block
